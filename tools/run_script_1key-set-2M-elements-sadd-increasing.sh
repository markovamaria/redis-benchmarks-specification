...
do
    echo $i
    redis-benchmarks-spec-client-runner --db_server_host localhost --db_server_port 6379 --test memtier_benchmark-1key-set-2M-elements-sadd-increasing.yml --client_aggregated_results_folder ./run_"$i" --flushall_on_every_test_start --flushall_on_every_test_end |& tee -a client_runs_"$EXP_RUNS".log
done
